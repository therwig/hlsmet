...
int alg_test(const char* dumpfile="") {
    ...
    for (int i=0; i<NTEST; ++i){
		std::cout<<std::endl;
		std::cout<<"...::: Start "<<i<<" Event :::..."<<std::endl;
        ...
        // compare outputs
        //
		if(DEBUG) std::cout<< " >>> Compare Outputs <<<"<<std::endl;
        if(DEBUG) std::cout<< "   HW 58 bits output: "<<output<<std::endl;
        int16_t val_pt_hw;
        int16_t val_phi_hw;
        val_pt_hw = output(57,42);
        val_phi_hw = output(41,32);
		if(val_phi_hw > (1<<9)) val_phi_hw = val_phi_hw - (1<<10); //Convert overflow of minus value.
